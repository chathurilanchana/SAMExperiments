...
The SAM CLI installs dependencies defined in `src/OrderApi/OrderApi.csproj`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
OrderApi$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```
...
```bash
OrderApi$ sam logs -n NetCodeWebAPIServerless --stack-name OrderApi --tail
```
...
```bash
OrderApi$ dotnet test test/OrderApi.Tests/OrderApi.Tests.csproj 
```
